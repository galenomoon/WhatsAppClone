...
export class WhatsAppController { //Exportando classe (neste caso o módulo que está utilizando dessa exportação é o app.js)
    ...
    // ================ TODAS AS AÇÕES REFERENTES A AUTENTICAÇÃO E AO USUÁRIO AUTENTICADO ================ 
    initAuth() {

        this._firebase.initAuth().then((response) => { //1) Ao autenticar, receberá as respostas dessa autenticação
            this._user = new User(response.user.email) //2) Selecionaremos o EMAIL recebido na response e mandar para o contrutor User em User.js
            this._user.on("datachange", data => { //Recebe o retorno do toJSON
                //Insere o nome do usuário no título da aba
                document.querySelector('title').innerHTML = data.name + ' - WhatsApp 2'
            ...
            this._user.save().then(() => {
                    this.el.appContent.css({
                        display: 'flex' //Libera a tela após Sing Up
                    })
                })
                    ...
                    view.querySelector('.btn-message-send').on('click', e => {
                        ...
                        Chat.createIfNotExists(this._user.email, message.content.email).then(chat => {
                            ...
                            contact.on('datachange', data => {
                                ...
                                contact.chatId = chat.id; //Definiu um ID no usuário adicionado

                                this._user.addContact(contact)

                                this._user.chatId = chat.id //Define o mesmo ID no MEU perfil de usuário
    ...
    initEvents() {
        ...
        this.el.btnSavePanelEditProfile.on('click', e => {
            ...
            this._user.save().then(() => { //REGISTRANDO NOVO NOME NO DB
        ...
        this.el.btnSendPicture.on('click', e => { // ENVIANDO FOTO TIRADA
            ...
            let filename = `cameraWhatsApp${Date.now()}.${ext}`;
            console.log(result)
        ...
        this.el.btnReshootPanelCamera.on('click', e => { // TIRAR FOTO NOVAMENTE
        ...
        this.el.btnAttachDocument.on('click', e => {
            ...
            this.el.inputDocument.click() //Abre aba de seleção de arquivos do SO
        });

        this.el.inputDocument.on('change', e => { //ENVIO DO DOCUMENTO
        ...
        this.el.btnSendMicrophone.on('click', e => {
            ...
            this._microphoneController.on('recordtimer', timer => {
                this.el.recordMicrophoneTimer.innerHTML = this.formatRecordTime(timer)
            })
        })
        ...
        this.el.btnFinishMicrophone.on('click', e => { //FINALIZAR GRAVAÇÃO e MANDAR AUDIO
        ...
        this.el.btnSend.on('click', e => { // BTN DE ENVIAR MENSAGEM
            ...
            Message.send(
                this._contactActive.chatId, //Chat ao qual a conversa ocorrerá [CHATID]
                this._user.email, // Quem está enviando [FROM]
                'text', // Tipo de mensagem [TYPE]
                this.el.inputText.innerHTML // Conteúdo da mensagem [CONTENT]
            );
